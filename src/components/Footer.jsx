...
export default function Footer() {
  ...
  const safeFacebook = (BUSINESS.social?.facebook ?? '').trim()
  const safeInstagram = (BUSINESS.social?.instagram ?? '').trim()
    ...
    <footer className="relative w-full bg-ink border-t border-white/10 pt-fluid-xl pb-fluid-sm px-fluid-sm text-white overflow-hidden">
      <div className="relative z-10 max-w-[1800px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-fluid-md mb-fluid-lg">
        ...
        <div className="md:col-span-2 pr-fluid-sm">
          ...
          <p className="text-fluid-xs text-white/55 max-w-md leading-relaxed tracking-widest uppercase">{t('footer.statement')}</p>
        </div>
      ...
      <div className="relative z-10 max-w-[1800px] mx-auto flex flex-col md:flex-row justify-between items-center text-[clamp(8px,1vw,10px)] text-white/45 tracking-widest border-t border-white/10 pt-fluid-sm">
        ...
        {(safeFacebook || safeInstagram) && (
          <div className="flex space-x-6 mt-4 md:mt-0">
            {safeFacebook && (
              <a href={safeFacebook} className="hover:text-accent-cyan transition-colors" target="_blank" rel="noreferrer">
                FACEBOOK
              </a>
            )}
            {safeInstagram && (
              <a href={safeInstagram} className="hover:text-accent-cyan transition-colors" target="_blank" rel="noreferrer">
                INSTAGRAM
              </a>
            )}
          </div>
